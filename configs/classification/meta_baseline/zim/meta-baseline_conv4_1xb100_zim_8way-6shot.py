...
num_ways = 8 # classes in support set
num_shots = 6 # samples per each class in support set
num_queries = 4 # how many queries are to be made
...
data = dict(
    ...
    workers_per_gpu=8,
    train=dict(
        # `dataset_wrappers`: the way the pairs are generated, so-called `episodes`
        type='EpisodicDataset',
        # number of total episodes = length of dataset wrapper
        num_episodes=100000,
        # each call of `__getitem__` will return
        # {'support_data': [(num_ways * num_shots) images],
        #  'query_data': [(num_ways * num_queries) images]}
        num_ways=6, # different classes
        num_shots=5, # number of support shots for each class
        num_queries=5, # number of query shots for each class
        dataset=dict(
            type='ZIMDataset',
            data_prefix='',
...
# runtime settings
# using IterBasedRunner because most papers or related codebases
# use number of iterations in their as default
# converting Runners from EpochBased to IterBased: https://github.com/open-mmlab/mmfewshot/issues/15
